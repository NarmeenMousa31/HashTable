...
/// Calculate an optimal table size based on the number of elements to minimize collisions
int calculate_optimal_table_size(int num_elements)
{
    int ideal_size = (num_elements * 2) + 20 ;
    while (1)
    {
        int prime = 1;
        for (int i = 2; i * i <= ideal_size; i++)
        {
            if (ideal_size % i == 0)
            {
                prime = 0;
...
void insert(table tab, int size, char *key, char *meanings, int method, int *collision_counter)
{
    ...
    while (strcmp(tab[location].value, "---") != 0 && strcmp(tab[location].value, key) != 0)
    {
        ...
        i++;
        location = (method == QUADRATIC_PROBING) ? quad(i, key, size) : double_hash(i, key, size);
    }
...
void rehash_function(table oldt, table newt, int method, int *collision_counter)
{
    ...
    for (i = 0; i < TABLE_SIZE; i++)
    {
        ...
        {
            // Assuming oldt[i].meaning is a string. If it's an array, you may need to adjust this.
...
void print_to_file(table tab, int size, FILE *fw)
{
    ...
    for (j = 0; j < size; j++)
    {
        // Print the word and its index to the file
        fprintf(fw, "\n\nWORD : (%d)!!!! %s", j, tab[j].value);
        fprintf(fw, "MEANINGS:");

        // Print all meanings associated with the word to the file
        for (int k = 0; k < tab[j].status; k++)
        {
            fprintf(fw, "\n\t- %s", tab[j].meaning[k]);
        }
    }
...
void change_table(table tab)
{
    ...
    for (j = 0; j < TABLE_SIZE; j++)
    {
        ...
        } //end of if there are empty spots at tab
    }     //end of for loop over the table
}         //end of function
    ...
    while (1)
    {
        ...
        case 2:
            ...
            if (tableChoice == 1 || tableChoice == 2)
            {
                ...
                table t = (tableChoice == 1) ? t_quad : t_double;
        ...
        case 4:
            ...
            collision = 0;
// Insert the word using Quadratic Probing
...
// Reset the collision counter and insert the word using Double Hashing
            collision = 0;
